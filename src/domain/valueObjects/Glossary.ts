 ...
 * ```
 *
 * ### チップが誤答肢にしか出ないのは正常（2026-09-10 に測定・裁定）
 *
 * 「用語チップが不正解の選択肢にしか出ておらず、出題テーマを誤読しかけた」という報告が
 * 2 ペルソナから独立に出た（pg-001 の 権威DNS/ゾーン、em-002 の L4）。測ったところ
 * **110/756（14.5%）** の設問がこの形で、追加した語の多く（エニーキャスト・BYOIP・526エラー）も該当する。
 *
 * → **仕様として据え置く。** 学習者は 4 肢すべてを読んで選ぶので、誤答肢の中の語を説明するチップは
 *   そのまま役に立っている。またチップは回答前に全肢を走査して出るので、
 *   「チップが付いている肢が誤答」という手がかりにはならない（付き方は語の有無だけで決まる）。
 *
 * ### 逆向き（正解肢にだけ出る）も測った（2026-09-13）
 *
 * 「チップが正解肢にしか出ない語なので、選択肢を読む前の手がかりになっていた」という報告が出た（wp-017）。
 * 設問文・ヒントに出る語を除いて 756 問を数えた結果:
 *
 * ```
 * 正解肢にだけ出る   32 問
 * 誤答にだけ出る    132 問   ← 4 倍
 ...
 * ```
 *
 * → **チップは正解の手がかりにならない。** むしろ 4 倍の偏りで誤答側に出るので、
 *   チップを頼りに選ぶ受験者は**外す方に誘導される**。個別の設問で目立つことはあっても、
 *   系統的な漏れではない。**割合を測らずに1問の印象で判断すると、逆向きの結論になる。**
 *
 * em-002 は L4 が**意図的に主題外の誤答**（DDoS のスコープ）にだけ出る例で、チップがその肢を目立たせる。
 * これは語の側の問題ではなく、その設問固有の巡り合わせ。語を消すと sc-010（正解肢）と sc-016 が困る。
 *
