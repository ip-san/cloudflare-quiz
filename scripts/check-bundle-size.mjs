...
// 【2026-09-16 引き上げ】initial 2700→2850 / totalAll 2800→2950。
// **カテゴリ追加ではない。設問数は756問のまま。** ヒント層・誤答層の掃引で
// 既存の文章を書き直した結果、quizzes.json が 2,534KB → 2,641KB（+107KB）に増えた
// （ヒントを軸の説明に、誤答を4肢そろえる形に、解説に根拠を追加）。
// オフラインで全問解ける前提は変えていないので、分割ではなく閾値の引き上げを選んだ。
// gzip 後の初回訪問は 668KB（quiz-data 472KB）で、2回目以降は Service Worker が返す。
// 余裕は約100KB＝2カテゴリ分。**次にここが赤くなったら、今度こそ分割を検討すること。**
const LIMITS = {
  totalInitial: 2850, // 初期ロード合計（現状 ~2750KB。大半は quiz-data の 2075KB）
  singleChunk: 400, // 単一チャンク（quiz-data は問題データ本体なので別枠）
  totalAll: 2950, // 全チャンク合計（現状 ~2834KB）
}
