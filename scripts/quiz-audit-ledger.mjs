 ...
 *
 * ### --note にバックティックを書くときは単一引用符で囲むこと（2026-09-13 に踏んだ）
 *
 * 二重引用符で囲むと、シェルが `Full` をコマンド置換として解釈し、**注記から語が消える。**
 * 「Full モードとの対比」が「 との対比」になって記録された。エラーは出ず、記録は成功する。
 *
 *   悪い例  --note "Full との対比に差し替え"      → `Full` が消える（バックティック付きの場合）
 *   良い例  --note 'Full モードとの対比に差し替え'  → そのまま入る
 *
 * 気づいたあと 63 件の注記を走査して、欠落しているのはこの1件だけだと確認した。
 *
 * ### 層の定義（既存の3台帳と1対1、＋ヒント）
 *
...
export function statusLines(quizzes, ledger) {
  ...
  lines.push('※ ◐ の層には「基準点」の記録がある。検証済みではなく、変わった分を差分で見るための出発点（hint 層）')
  lines.push(
