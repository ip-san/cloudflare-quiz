---
name: playtest
description: 一般ユーザーを模したエージェントが実 PWA のクイズをプレイし、分かりにくさ・学び改善のリクエストを出し、専門家チームがレビュー・改善するゲート。プレイテスト、ユーザーテスト、playtest、分かりにくさ、学習改善
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, Agent, Skill
argument-hint: "[--persona ...] [--count N] [--report-only] [--no-build] [--progressive [N]]"
---

# Playtest Skill — 模擬ユーザー・プレイテスト & 専門家レビューゲート

模擬ユーザーエージェントが**実 PWA をブラウザで操作**してクイズをプレイし、
「分かりにくい / 学びにくい」を**ユーザーの声**として出す。それを**専門家チームが妥当性検証**してから
クイズ内容・学習設計を改善し、UX/UI は報告する。ユーザーの不満を鵜呑みにせず、
事実正確性（`quiz:lint`/`quiz:fact-check`）と教育設計を守るのが「ゲート」の役割。

参照: `personas.md`（ペルソナ定義）, `feedback-schema.md`（JSON 契約）。

## ペルソナ別の進捗（2026-09-08 時点）

```
                  プレイ済み
beginner          156/200 (78%)
busy-intermediate 114/333 (34%)
reviewer           89/223 (40%)
```

**最新の数字は `node scripts/playtest-coverage.mjs status` で見ること。この表はすぐ古くなる。**

### ペルソナごとに見つけるものが違う（2026-09-07〜08 の 5 バッチで確定）

- **初学者**: 用語チップの穴。「解説に生の API 名が並ぶ」「エニーキャストが未登録」など。
  ヒントは効いていて**正解はできる**が理解が置き去りになる型を挙げる
- **中級者**: 「実務では都度確認する数値の暗記」。多くは 08-30 の裁定どおり据え置きになるが、
  **同じ設問を 3 人以上が誤答したら設問の構成を疑うこと**（as-018 は 4 回目で 1 軸への組み替えに至った）
- **査読者**: **形式で当たる設問**。長さ・絶対語の一様化では拾えない型で、査読者だけが見つける。
  「設問文の前置きが正解の構造を先取り」（cx-013 / cx-016 / dn-009 / dl-008 の 4 例）、
  「誤答だけに括弧注釈が付く」（ct-017）。査読者を回す価値はここにある

  この型は **2026-09-09 に機械での選別を試して失敗した**（語の重なりでは 21.6%、偶然の 25% 以下。
  既知 4 件も 755 件中 92〜427 位。負の結果 8 件目）。前置きは正解と語ではなく**論理の形**を共有するため。
  **査読者を回す以外に見つける手段が無い。バッチの配分は査読者に寄せること。**

**この差が、いま何が残っているかを示している。**

- **査読者が39問で指摘ゼロ**なのは、誤答2,268肢・正解756問・図876枚の
  全数監査が効いている証拠。出典と論理の整合はもう問題ではない
- **初学者だけが3倍近くつまずく。** 残っているのは「事実が正しいか」ではなく
  「初めての人に説明が届くか」

### 何が言えて、何が言えないか

**言える:** 事実の誤りを潰す作業と、説明を届かせる作業は**別の作業**であり、
前者を100%やっても後者は自動的には終わらない。

**言えない:** 初級の詰まり率が今後どこまで下がるか。
書き直した6問は3周目で 0/6 になったが、
まだテストしていない初級問題が178問ある（掃引は全200問に当てたが、
掃引で直したものが実際に届くかは**プレイテストでしか確かめられない**）。

### 用語集の中身はプレイテストからしか増えない（実測）

**頻度では「初学者が知らない語」を特定できない。**

2026-08-30、初級200問から「4問以上に出て用語集に無い語」を機械抽出したところ
**149語**が挙がったが、上位はこうだった:

```
86問 Cloudflare / 56問 リクエスト / 47問 Worker / 32問 アクセス
31問 ファイル / 29問 ユーザー / 24問 ブラウザ / 23問 サービス
```

**普通の語ばかりで、詰まらせる語ではない。**
一方でテスターが実際に詰まった語は、頻度では埋もれている:

```
クレデンシャルスタッフィング / 負のセキュリティモデル / OpenAPI / Terraform
```

このプロジェクトで**機械抽出が使えなかったのは5回目**
（初級解説の未説明語 124/200、図にしか出ない語 756/756、
絶対語の一様化、コード比較の設問、そして今回）。

### したがって、これは一度きりの作業ではなく維持のループ

```
プレイテスト → 「この語が分からなかった」 → docs で裏を取る → 用語集へ
```

1周で数語ずつ増える。**収束を急がないこと。**
語を増やすために推測で定義を書けば、その瞬間に事実の注入になる。

### 3周の実測

```
1周目（用語集なし）      詰まり 5/8
2周目（用語集あり）      詰まり 3/8  ← 説明なしで発見・使用できた
3周目（収録範囲を拡大）  詰まり 4/8  ← 旧3件のうち2件解消。残りは新規問題の新しい語
```

3周目で増えたのは、**未テストの問題を混ぜたから**。同じ8問を繰り返せば
数字は下がるが、それは「その8問が直った」以上のことを意味しない。

## 引数

- `--persona <name>`: 単一ペルソナのみ実行（既定は3ペルソナ全部）
- `--count N`: 1ペルソナあたりのプレイ問題数（既定: beginner 9 / 他 10）
- `--report-only`: 改善を適用せず、リクエストと専門家判定の**報告のみ**（dry-run）
- `--no-build`: 既に preview サーバーが起動済みの場合、ビルド/起動をスキップ
- `--progressive [N]`: プログレッシブ・カバレッジモード（下記）。未テストの問題を N 問（既定5）狙って1バッチ実行

## プログレッシブ・カバレッジモード（`--progressive`）

全クイズを「ユーザー視点で1問ずつ」テストし切るためのモード。毎回**未テストの問題**を狙うので、
全問を1周するまで毎バッチ新しい発見がある（空振り・同じ不満の再生産を避ける）。記録は
`.claude/playtest-coverage.json`（git追跡で進捗が永続）。

1. `node scripts/playtest-coverage.mjs status` で進捗確認
2. `node scripts/playtest-coverage.mjs next [N]` で「次に testする persona + 問題ID + `?q=` deep link」を取得（残数最多ペルソナを自動選択）
3. `bun run build && (bun run preview &)`（`--no-build` 時はスキップ）
4. `user-simulator`（取得した persona）を**ターゲットモード**で起動。プロンプトに `ids` と `deepLinks` を渡し、指定問題だけを deep link で順にプレイさせる（各 item に `quizId` 直記録、最終 `played:[{id,outcome}]` を返す）
5. `node scripts/playtest-resolve.mjs` → ドメイン別 `learning-experience-reviewer` で検証 → `playtest-apply.mjs` で承認分適用 → 事実変更時は該当カテゴリで `bun run quiz:lint:dry`/`bun run quiz:fact-check` 再検証 → `bun run quiz:randomize && bun run quiz:check && bun run test`
6. `node scripts/playtest-coverage.mjs mark-batch <played.json>` でテスト済みを記録
7. preview 停止。`auto/playtest-cov-<日時>` ブランチにコミット（coverage 更新 + 改善）。**push/PR はしない**
8. 報告: 今回のカバレッジ進捗（例 beginner 40/254）、採用/却下、UX課題、残数

**1周完了**（全問 covered）したら維持モードに移行する（下記 `retest`）。
なお1周する前でも、書き換えた問題は `retest` で随時戻すこと。

### 【最重要】検証のたびにポートを変える — Service Worker が古い内容を配る

このアプリは PWA で、`dist/sw.js` が quiz データのバンドルをプリキャッシュする
（`registerType: 'autoUpdate'`）。**同じポートで再テストすると、
Service Worker が修正前の内容を配り続ける。**

2026-08-30 に実際に起きた:

```
書き直した6問を再プレイ → 5問は新しい内容、ac-002 だけ前の版が表示された
同一セッション内で「通常リロード→旧内容 / ハードリロード→新内容」を再現
前回のプレイでキャッシュ済みだったのが ac-002 だけで、
未キャッシュの5問は新規フェッチで新しい内容を拾っていた
```

**まだら状に古くなる**ので、結果を見ても気づけない。実際このとき私は
「テスターが前回の内容を報告した」と誤って結論し、本人にそう伝えた。
テスターが `get_page_text` の実物とハードリロードでの再現を示して訂正してくれた。

**対処:** Service Worker のスコープはオリジン単位なので、ポートを変えれば持ち越さない。

```bash
bun run build
bun run preview --port 4181          # 前回と違うポート
PLAYTEST_BASE_URL=http://localhost:4181/cloudflare-quiz/ \
  node scripts/playtest-coverage.mjs retest 12
```

`check-preview-fresh.mjs` は**ビルドの新しさしか見ない。**
ビルドが最新でも Service Worker が古い内容を配ることがある。

### 再テストのとき、前回の結果を伝えないこと

上の件で私は最初「プロンプトに『前回この6問では全問で何かしら詰まっていますが、
それを再現する必要はありません』と書いたのが原因」と結論した。
**これは誤りだった**（原因は Service Worker のキャッシュ）。

ただし助言そのものは有効なので残す。「再現する必要はない」という打ち消しも
前回の内容を伝えているので、再テストでは「再テストです」とだけ伝え、
検証したい観点は**一般の観点として**書くこと。

**そして報告を鵜呑みにしないのと同じくらい、自分の結論も疑うこと。**
私は現物を確認したうえで「テスターが誤っている」と結論したが、
テスターのほうが正しかった。**確認して食い違ったら、まず自分の観測条件を疑う。**

### 現物の確かめ方も間違える

上の `ac-002` を検証したとき、私は配信ビルドから
「`ac-002` の位置から1800文字」を切り出して調べ、
**Rule type の定義は「なし」と誤判定した。**
実際にはその設問の領域は1583文字で、定義は含まれていた
（切り出しの起点がずれていたため範囲外になっていた）。

そこで止めていたら、**直っているものを「直す」**ところだった。

同じ形の誤りをこのプロジェクトで3回している:

```
2026-08-28  docs を文全体で grep → 改行と <GlossaryTooltip> で外れ、
            「記述が無い」と結論しかけた（2件）
2026-08-30  配信ビルドを固定長で切り出し → 範囲外になり「定義なし」と誤判定
```

**「無い」と結論する前に、探し方が当たっているかを別の方法で確かめること。**
短い語で引き直す、範囲を広げる、元データ（`src/data/quizzes.json`）と
配信物の両方で見る、など。

## 指紋は「本文」だけでは足りない — 用語集も画面を変える

2026-08-30、用語集に22語を足したあと再テスト対象を出したら、
**テスターが「語が分からない」と詰まった11問が一つも挙がらなかった。**

設問を1文字も変えていないので本文の指紋が動かず、カバレッジは
「現行内容でテスト済み」と言い続けていた。だが**画面に出るものは変わっている**。
用語バッジが7個増えた設問は、前に読んだときとは違う設問になっている。

これは指紋を手計算して嘘の記録を書いたのと同じ種類の嘘で、
**しかも自動なので誰も気づけない。**

→ 記録に `gt`（その設問に出る用語バッジの指紋）を持たせた。
   本文が同じでも `gt` が変われば stale になる（理由は `glossary`）。
   `--played-at <ref>` は Glossary.ts も同じ ref から読む。
   既存の記録へは `backfill-gt <ref> <batchfile>` で、
   **そのバッチをプレイした時点の用語集から**埋める（全記録に一律で埋めると嘘になる）。

`gt` を持たない古い記録は判定しようがないので黙って通している。
**嘘はつかないが、確認済みとも言っていない**という状態。

### 一般化

**「画面に出るものを変える経路」を全部数えること。** 本文だけを見張るのでは足りない。
今のところ本文・図・難易度・用語集の4つ。UI 側に同じ性質のものを足すときは、
指紋に入れるかどうかを必ず決めること。

## 用語集に何を入れるか — 頻度で選ぶと外れる

2026-08-30、初級36問（3ペルソナ）で friction 14件。**うち11件が同じ原因**で、
「意味の分からない語が選択肢に出るので、何を比べさせられているのか分からない」だった。

決定的だったのはテスターのこの一言:

> 選択肢を読んでいる間はLayer4とかFWaaSとか知らない言葉だらけで、正直何を
> 比べさせられているのか分からなかった。**用語グロッサリーに出ていた
> 『Gateway』はむしろ既に分かる言葉だった**

初期の23語は**コーパスの出現頻度**で選んでいた（オリジン40問、エッジ24問…）。
頻度が高い語は、その分野の文章に何度も出てくる語 ＝ **学習者が既に知っている語**に偏る。
実際に足を止めるのは、1〜2問にしか出ない専門語のほうだった。

→ **収録の基準は「頻度」ではなく「実測で詰まったこと」。** 頻度は使わない。
   ただし逆向きの過剰反応もしないこと。1人が「知っていた」と言った語を消すのは n=1 での消し過ぎ。

### 候補を機械で挙げる — 3回目でようやく当たった（`bun run quiz:acronyms`）

  1回目  頻度で並べた  → Cloudflare / リクエスト / ファイル が上位。
                        **頻度が高い語は学習者が既に知っている語**に偏る（負の結果）
  2回目  稀少さで並べた → 長いカタカナ複合語ばかり（「オブジェクトストレージサービス」等）。
                        語形から意味が読めるので詰まらない（負の結果）
  3回目  当たり。ただし**当て方を変えた**

詰まった語（Layer4 / FWaaS / DEX / DLQ）に共通するのは**非合成性** ――
部分から意味が推測できないこと。略語はこれを構造的に満たす。

だが決め手はそこではない。**何が難しいかを当てるのをやめたこと**が効いた。
難易度は主観で、私は実際に外している（初期23語は頻度で選んで既知の語ばかりだった）。

代わりに客観的な性質だけを見る: **その設問の中で展開されているか。**
展開が無ければ、その略語を知らない読み手には解決の手立てが無い。
難しいかどうかは判定していない。**解決可能かどうかだけ**を見ている。

初級で115件 / 44語。SKIP リストが唯一の主観なので、外したものを明示してある。
**迷ったら外さないこと**（出しすぎるほうが、見落とすより害が小さい）。

### docs が展開しているかを基準にする（`--docs-expanded`）

「難しそうか」を私が判定するのはやめる。代わりに:

> **Cloudflare の docs が、技術者向けの文章でわざわざ展開しているか。**

docs が展開する語は、技術者にも自明でないと docs 自身が判断した語。
このアプリの読み手はそれより初学者なので、**docs が展開するならこちらでも要る**。
主観が入らず、根拠が外にある。

2026-08-30 に CLI / BGP / AWS の3語がこれで入った。

### 捕獲した文字列をそのまま使うと事実を捏造する

同じ抽出で SSL と SSH も挙がったが、**どちらも展開ではなかった**:

    SSL   「TLS (SSL)」        …… SSL は TLS の**別名**として書かれているだけ
    SSH   「Infrastructure (SSH)」 …… 製品の見出し語。表の行

正規表現の捕獲を信じて「Secure Sockets Layer」などと書いていれば、
docs に無い事実を用語集に注入していた（em-009 と同じ型）。
用語集は該当する全問に出るので、影響範囲は1問の誤りより広い。

→ `--docs-expanded` は**展開文字列を出さない。docs の行をそのまま出す。**
   捕獲を信じる余地を無くしてある。行を読まずに収録しないこと。

### 出た候補を機械的に入れてはいけない

「知らない読み手には解けない」は言えるが「**実際に詰まる**」は言えない。
収録の可否はプレイテストで確かめること。

例外は `--corpus-expanded`。**コーパスが別の設問で既に展開を約束している**略語で、
出た設問では展開されていないもの。これは判断が要らない ――
難しさを推測しているのではなく、コーパス自身の展開を配っているだけ。
2026-08-30 に SFU / TURN / CASB の3語が該当した（いずれも docs で裏取り済み）。

### 収録の前に必ずやること

1. **docs で裏を取る。** 推測で定義を書くのは `em-009` の事実注入と同じ失敗。
   docs が説明していない部分は**書かない**（例: DS と DNSKEY の鍵連鎖の詳細は
   learning センター側にしか無いので、docs にある「DSはレジストラ側」までで止めた）。
2. **コーパスの実表記を数える。** docs の表記と設問の表記は違う。
   `Layer 4` と `レイヤー4` は両方あった。片方だけ入れると片方は死ぬ。
3. **英字略語は語境界を確認する。**

### `DEX` が `CREATE INDEX` に当たった

`DEX`（13問で使う）を入れようとして、D1 の設問にある `CREATE INDEX` の
**IN`DEX`** に一致することに気づいた。危なかったのは、**本文側では出ていなかった**こと。
本文はコード片を除外するので偶然逃れていた。だがチップ側は生の文字列を
`includes` で見ていたので**確実に出ていた**。

「本文で試して出なかったから大丈夫」は根拠にならない。**同じ語を照合している場所を
全部数えること。** 現在は `Glossary.ts` の `hasTerm` / `indexOfTerm` に一本化し、
`Glossary.test.ts` が語境界を守っている。

### 実機確認は必ず狭い画面でもやる

用語集のバグは**3回とも実ブラウザでしか出ていない**。単体テストは全部通っていた。

  1回目  選択肢は `<button>` なので、中に用語のボタンを入れると回答が確定した
  2回目  ポップオーバーが `bottom-full` で、見出しの語では画面外へ出た
  3回目  ポップオーバーが `left-0` で幅 16rem。**画面の右寄りの語で右へはみ出した**

3回目は 1568px の画面では見つからない。右に余白があるので収まってしまう。
これは PWA で、トップに「スマホでいつでも」と書いてある。**広い画面での確認は確認になっていない。**

→ 実機確認は 390px 幅でも行い、**行の右端にあるチップ**を開くこと。
   横位置の計算は `clampShift` に切り出してテストがある。

### 語彙で解けないものを語彙に混ぜない

同じ回で出た friction のうち、次は用語集の仕事ではなかった:

- `hw-010` **ヒントが選択肢A・Bの両方に等しく当てはまり、絞り込みに使えていなかった。**
  ヒントとしての機能を果たしていないので、対象（CI/CDではなくアプリケーション処理）で
  切れるように書き直した
- `kv-001` 選択肢が固有名詞4つだけで、ヒントが一般論のみ。判断の軸を名指しした
- `lb-010` 暗記型。docs に5種類と書いてあるので、その数をヒントに出して
  「〜のみ」の選択肢を正当に外せるようにした
- `lb-008` **docs は非互換を制約として述べるだけで、理由を書いていない。**
  理由を創作するのは事実注入なので、解説に
  「ドキュメントは理由を説明していない、覚えておく制約」と明記した。
  **答えられない理由は、答えられないと書く。**
- `dq-003` コードを書かない読み手が API 名に馴染めない件は、ヒントが既に
  判断手順を与えており実際に正答できている。仕様どおりとして据え置き

### 報告は核だけ拾う

`hw-002` は「選択肢とヒントに ORM とバインディングが未定義」と報告されたが、
**バインディングは hw-002 に存在しない**（テスターが直前の kv-013 と混同していた）。
未定義なのは ORM だけ。指摘の核は正しかったので拾ったが、
**語の存在は必ずコーパスで確かめてから直すこと。**

## 維持モード（`retest`）— 内容を書き換えた問題を戻す

プログレッシブモードは**未テストの問題**を狙う。だが問題は書き換わる。
テスト済みの問題を直したら、その「テスト済み」は**現行内容についての保証ではなくなる**。

```bash
node scripts/playtest-coverage.mjs stale            # 再テストが要るものの一覧
node scripts/playtest-coverage.mjs retest [N] [persona]   # next と同じ形で出す
```

`retest` の出力は `next` と同じ形なので、user-simulator へ渡すプロンプトは
プログレッシブモードと同じ組み立てで済む。

### 再テストが要る2種類

| reason | 意味 | 対処 |
|---|---|---|
| `changed` | テスト後に中身が変わった | 再プレイ |
| `no-fp` | 指紋が無く、現行内容でテストされたか**確かめようがない** | 再プレイ |

**`no-fp` を「判定不能だから除く」にしてはいけない。**
それは「検証できない記録を信用する」ことと同じで、台帳に静かな盲点を残す。
2026-08-29 に7件を調べたところ、うち3件は実際に内容が変わっていた。

### `orphaned` — 難易度が変わってしまった記録

記録した後に `difficulty` が変わると、そのペルソナで再プレイしても
**進捗の分母に入らない**。`retest` はこれを `orphaned` として分けて出す。
担当ペルソナを変えて回すこと。

---

### 誤った観測が出たときの記録の分け方

観測が古いキャッシュ由来だったと分かったとき、記録を消すか残すかで往復した。
**両方を別の場所に持たせるのが正しい。**

- **`played` の `outcome` は「現在の内容が詰まりを起こすか」を記録する欄。**
  旧内容に対する詰まりは `clean` にする。台帳（`playtest-coverage.json`）は
  「現行内容でテスト済み・詰まりなし」を持つべきだから
- **`items` は観測された事実として残す。** `supersededBy` に理由を書く。
  消すと「何が起きて、なぜ誤解を招いたか」が失われる

どちらか一方では足りない。台帳は判定を、ファイルは経緯を持つ。

なお 2026-08-30 のこの往復は、私が担当者に「再Writeは不要です」とだけ伝えて
**何をどう残してほしいかを言わなかった**のが原因だった。

## 【最重要】記録は編集より**先**

```bash
node scripts/playtest-coverage.mjs mark-batch <played.json> [--played-at <ref>]
```

**プレイ結果を適用する前に記録する。** 逆にすると、プレイ後に直した問題の指紋が
**直したあとの値**で記録され、台帳が「現行内容でテスト済み」と嘘をつく。

順番を間違えたら `--played-at <ref>` で直せる。`<ref>` は
**プレイ時に配信していたビルドの元コミット**（プレイテストのコミット自体ではなく、
その改善を入れる前）。

**指紋を手で計算し直さないこと。** 2026-08-29 に手計算で埋めようとして、
区切り文字をツールと変えてしまい違う値を書き込んだ。
`--played-at` と `backfill-fp <ref> <id...>` は、どちらも
**同じ fingerprint 関数に別の入力を渡すだけ**なので、計算がずれる余地が無い。

## 前提チェック（最初に実行）

1. claude-in-chrome（ブラウザ自動化）MCP が利用可能か確認。不可なら「ブラウザ MCP 未接続」と報告して中止
   （データレベル代替は本スキルの対象外。実 UI 体験が目的のため）
2. `mkdir -p .claude/tmp/playtest`

## Phase A: プレイスルー（3ペルソナ並列・実ブラウザ）

1. `--no-build` でなければアプリを用意:
   ```bash
   bun run build && (bun run preview &)   # http://localhost:4173/cloudflare-quiz/
   ```
   起動待ち（`until curl -sf http://localhost:4173/cloudflare-quiz/ >/dev/null; do sleep 1; done` 相当）

   **`--no-build` の場合でも、ペルソナを起動する前に必ず次を実行すること:**
   ```bash
   node scripts/check-preview-fresh.mjs   # exit 1 なら再ビルドするまで進まない
   ```
   preview は `dist/` を配信するため、ビルドが古いと模擬ユーザーは
   **修正済みの問題を修正前の状態でプレイ**する。2026-08-26 に実際これが起き、
   ac-008 / ac-009 / ag-001 の3件が「修正済みなのに再指摘」として戻ってきた。
   結果からは「古いビルドを見ていた」と「本当にまだ直っていない」を区別できないので、
   走らせる前に落とす。
2. 対象ペルソナごとに `user-simulator` を **同一メッセージ内で同時に** `run_in_background: true` 起動:
   ```
   Agent(subagent_type: "user-simulator", model: "sonnet",
         prompt: "persona=<name>。personas.md と feedback-schema.md に従い実 PWA をプレイし
                  requests-<name>.json を書く。count=<N>。")
   ```
3. 全エージェントの完了通知を待つ → `requests-<persona>.json` が出力される

## Phase B: 集約・名寄せ（決定論的）

```bash
node scripts/playtest-resolve.mjs   # → requests.json（domain別・quizId名寄せ・stats）
```
`unresolved` があれば Phase C の reviewer が Grep で手当てする。

## Phase C: 専門家レビューゲート（ドメイン別並列）

`requests.json` の `byDomain` に項目があるドメインごとに `learning-experience-reviewer` を並列起動:
```
Agent(subagent_type: "learning-experience-reviewer", model: "sonnet",
      prompt: "domain=<content|learning|ux>。requests.json の byDomain[domain] を検証し
               verdicts-<domain>.json を書く。content/learning は事実裏取り、ux は report-only。")
```
- content/learning: accept/modify/reject + 具体 change を判定（事実は変えない）
- ux: report-only（`change` は null、ルーティング先を明記）

## Phase D: 適用 + 事実安全網（`--report-only` なら skip）

```bash
node scripts/playtest-apply.mjs            # accept/modify を quizzes.json へ（from不一致は安全スキップ）
bun run quiz:randomize && bun run quiz:check && bun run test
```
適用で**事実に触れた変更**があれば、該当カテゴリについて `bun run quiz:lint:dry` / `bun run quiz:fact-check` を
実行し、`.claude/tmp/docs/` のキャッシュと突き合わせて最終事実検証を行う。NG なら該当変更を巻き戻す。

## Phase E: レポート

```
## Playtest 結果
| ペルソナ | プレイ数 | 詰まり報告 | 主な声 |
|---------|---------|-----------|--------|
| ...     | ...     | ...       | ...    |

| ドメイン | accept | modify | reject | 代表例 |
|---------|--------|--------|--------|--------|
| content | ... |
| learning| ... |
| ux (報告のみ) | — | — | — | ルーティング先: /code-review 等 |

- quizzes.json 変更: N問（事実再検証 ✅）
- UX/UI 課題: M件（/code-review へ）
- 未解決名寄せ: K件
```

## quality-loop との統合

`/quality-loop --playtest` で Step 3.5 として本スキルを呼ぶ（コスト高のため既定では実行しない／月次・任意）。
UX 課題は quality-loop の Step 1（code-review）へ、内容修正は Step 3（最終ゲート）前に合流させる。

## モデル選択

- `user-simulator` / `learning-experience-reviewer`: Sonnet（文脈理解）

## 詰まり報告の切り分け: クリック回数の訴えはまず環境を疑う

ペルソナが「1回目のクリックで反応しない」「何回か押してやっと開いた」と報告したら、
**実装を読む前に、そのタブの `innerWidth` と `document.visibilityState` を測る。**

```js
JSON.stringify({innerW:innerWidth, innerH:innerHeight, vis:document.visibilityState})
```

`vis` が `hidden`（ウィンドウが背面・最小化）だと合成クリックがページに届かない。
1回目で前面化し2回目が届くので、**全問で再現する**。実装の欠陥に見える。

2026-09-12 に上級の最終バッチでこれが起きた。ヒントボタンの onClick は同期で門も待ちも無く、
`elementFromPoint` はボタン中心でボタン自身を返し、イベント列1回分でヒントが開いた。実装は正常だった。

ただし**同じ「反応しない」でも本物のことがある**。用語チップの当たり判定（2026-09-05、初級ペルソナが3回外した）
は座標のずれが実際にあった。**報告の文面だけでは区別できない。毎回測る。**

## 判定の取り込み: 完了メッセージを待ち、コミット前に diff を目で読む

2026-09-12 に、この2つを飛ばして**適用した内容と違うものをコミットした**。

**1. ファイルの存在は完了ではない。** `ls verdicts.json` が通っても、エージェントはまだ書いている
ことがある。書きかけでも JSON として妥当にパースできるので、**パースが通ることは完成の証明にならない。**
完了メッセージを受け取ってから読む。

**2. コミット前に `git diff` を読み、自分が書いた文字列がそこにあることを目で確かめる。**

```bash
git diff -- src/data/quizzes.json | grep -E '^[+-].*(hint|question)'
```

`playtest-apply.mjs` の「Applied 3」は、**コミット時点の内容の証明にならない**。
適用のあとに別のエージェントが同じファイルを書けば、`git add -A` がそちらを取り込む。
「適用した」と「コミットされた」は別の事実で、記録に残るのは後者だけ。

`learning-experience-reviewer` には「quizzes.json を直接編集しない」と明記してあるが、
tools に Bash があるので書けてしまい、実際に破られた。**規則を足しても防げない。防ぐのはこの確認だけ。**

## ブラウザを使う体は1つだけにする（2026-09-14〜15 に3回測った）

タブグループは全エージェントで共有されている。**複数の体が同時にブラウザを使うと、
後から作ったタブが即座に追い出される。**

```
3体同時  1体が実機操作を断念（09-13）
2体同時  1体が「作成したタブが直後に評決される」を4回確認して断念（09-15）
2体同時  別の回でも1体が断念（09-15）
```

**「2体まで」という目安を立てたが、2体でも競合した。実質1体である。**

### 代わりに何をするか

**内容監査（`quizzes.json` の全文を読んで判定する）で代替できる。** データ駆動のUIなので、
表示される内容と JSON は一致する（実際に1問開いて一致を確認した回がある）。

**検出率は落ちていない。** 実機で解いた回と内容監査だけの回で、欠陥の検出率はほぼ同じだった。

```
実機で解いた回    53% / 56% / 50% / 55% / 56%
内容監査だけの回  63% / 50%
```

### ただし失うものがある

**「ヒント無しで誤答し、ヒントを読んだ直後に正答へ変わった」という実測の裏付けが取れない。**
これは最も強い証拠で、「ヒントが本当に答えを渡している」ことを分析ではなく事実として示す。
`ob-017` では検査役が実際に外したことがヒントの無力さの決定的な証拠になった。

→ **1バッチにつき、ブラウザを使う体を1つだけ置く。** 残りは内容監査に回す。
   実測の裏付けが要る問（自分が直した問の再テストなど）を、ブラウザの体に優先して割り当てる。
