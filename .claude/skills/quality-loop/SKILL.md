...
- `--playtest`: Step 3.5 として `/playtest`（模擬ユーザー・プレイテスト）を実行する（コスト高のため既定OFF。月次・任意）
...
**スキップ条件:** `--skip-refine` フラグ、または `src/data/quizzes.json` に変更がなく、Step 0 の出力にも flagged 項目がなく、`bun run quiz:ledger` の「変わった」と「記録なし」が全層 0 の場合（新規設問は「記録なし」にしか出ない）
...
## Step 3.5: プレイテスト（`--playtest` 指定時のみ・Step 3 の後）

`/playtest --progressive`（引数省略時）を実行する。模擬ユーザーが実PWAを操作して分かりにくさを報告し、
専門家レビューゲートを通過した修正のみ `src/data/quizzes.json` に反映される。変更があれば再度 `bun run check` を実行する。
コストが高く claude-in-chrome MCP を要するため、既定では実行しない。詳細は `.claude/skills/playtest/SKILL.md` を参照。
