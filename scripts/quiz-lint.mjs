...
/** 読み手に判断を委ねる形 */
// 【2026-09-09 実測】この除外はヒントの 93.3%（705/756）に当たり、検査をほぼ空振りにしている。
// さらに HINT_TOKEN が英字と `code` しか見ないので、日本語の言い換えによる giveaway は原理的に見えない。
// 除外を外すと 20 件挙がるが大半は Worker / API / DNS のような一般語で偽陽性。
// **この検査が「クリーン」と言っても、ヒントが答えを渡していないことの保証にはならない。**
// ヒントと正解の 3-gram の重なりでの選別も試して失敗した（負の結果 9 件目。known-issues 09-09）。
// ヒントの giveaway は査読者ペルソナのプレイテストでしか見つからない。
/**
 * 設問の外（前の設問など）を参照するヒント。
 * 2026-09-14 に cs-017 で実際に出た:「…を、前の設問で整理したことを思い出しましょう。」
 * ターゲットモード（`?q=<id>` のディープリンク）には前の設問が存在せず、
 * 通常の出題でも直前の設問が同じ主題である保証は無い。**参照先が無いので何も渡らない。**
 * 全756問を走査してこの1件だけだったが、意味ではなく語で検出できる数少ない型なので規則にしておく。
 */
export const HINT_CROSS_REF = /(前の設問|前問|先ほどの設問|直前の設問|前の問題|次の設問|別の設問|さっき)/
