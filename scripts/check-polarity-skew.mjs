 ...
 *
 * 2026-09-15 に9問すべてを直して **0件にした**。誤答3つだけが制限を述べる形は、以後1件でも増えたら落ちる。
 *
 * 直し方は「誤答のうち1つ以上を**肯定形の誤り**に変える」。正解肢は触らない。
 * 全廃する必要はなく、偏りが消えればよい。実例:
 *
 *   ac-009  誤答1を Access のプライベートアプリケーション経路に差し替え。
 *           **手順を3つ並べる肢が正解と合わせて2つになり、手順の数を数える手が通らなくなった**
 *   dn-001  誤答2「BGP経路ハイジャックに対しては無防備」を
 *           「ゾーンへの署名は利用者側で行う」に。署名と DS レコード生成は Cloudflare 側（dnssec L75）
 *
...
// 「〜できない」系だけでなく「〜は不要」系、「〜に対しては無防備」系も同じ働きをする。
// どれも語族を足すたびに1問ずつ増えた（ac-009 / dn-001）。いずれも 1/1 で誤検知なし。
// **語の照合なので言い換えは拾えない。** 掃引の担当が目で見つけたものを都度足していく形。
// 誤答が「設定は不要」と言い、正解だけが手順を並べると、手順の多い肢を選ぶだけで当たる（ac-009）。
const RESTRICT =
  /(できない|されない|受け付けられない|失われる|対応していない|利用できない|使えない|含まれない|行わない|サポートしていない|反映されない|保証されない|不要|必要はない|必要ない|無防備|守れない|防げない)/
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // 2026-09-15 に9問すべてを直して0件にした。以後は増えたら落とす。
  process.exit(1)
}
