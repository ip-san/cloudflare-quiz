...
- 正解選択肢に含まれる固有のキーワード・API名・メソッド名・パラメータ名をそのまま引用していないか（パターンマッチだけで正解に到達できてしまう＝ giveaway）
- 良いヒントは「考える軸・方向性」を示す言い換えに留める（例: 正解の用語「単回使用」→ ヒントでは「使い捨て」のようなパラフレーズ、または問いかけの形で軸だけ示す）
- 判断基準: ヒントの中の語句を選択肢テキストと突き合わせ、正解選択肢だけに一意に出現する語（他の不正解には出てこない固有名詞・関数名）がそのまま使われていたら giveaway
- **severity:** info（言い換えへの修正を提案。事実誤りではないため必須修正ではない）
