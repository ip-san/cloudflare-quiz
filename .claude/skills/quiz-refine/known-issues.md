...
## 2026-08-26 wrongFeedback 2,268件のカバレッジ台帳（どこまで見て、どこから先を見ていないか）

`incomplete` 型（誤答がN点主張するのに wrongFeedback が一部しか反駁しない）の
走査結果。**「全部見た」とは言えないので、見た範囲と見ていない範囲を明示して残す。**

| 層 | 件数 | 状態 | 検出 |
|----|------|------|------|
| 候補（未被覆セグメントあり） | 261 | 全件レビュー | 1（dl-005） |
| Tier A 再分類後の新候補 | 312 | **全件レビュー・クローズ** | **0** |
| Tier B（語彙被覆で除外） | 650 | B1/B3/B4 がレビュー中 | 抽出30件で1（wk-012） |
| Tier A 由来の語彙被覆分 | 657 | 第4トランシェとして書き出し済み | 未着手 |
| 読点でも分割できない残り | 389 | **未検証のまま残す** | — |

**最大の学び: 分類の正しさと成果物の正しさは別物。**
Tier A の分類（「接続表現で分割して1セグメント」）は**7割が誤り**だったが、
そこから再分類した312件を全件読んで `incomplete` は**ゼロ**だった。
分割器の欠陥は「候補として挙がらなかった」形で現れたが、その裏で
wrongFeedback 側が実際には複数主張を潰していたため実害に至っていない。
例: `wk-004[1]`「文字列としてのみ取得可能で、JSONやバイナリを直接扱う手段はない」
に対し wf が `request.json()`/`arrayBuffer()`/`formData()` を挙げて両方を潰している。

**一方 Tier B で `wk-012[3]` が見つかったのは分類ではなく除外判定の甘さが原因。**
「wf と語彙が重なる＝被覆済み」としたが、wf が「CPU時間」に言及していても
反駁しているのは別の主張だった。**語彙が重なっていても反駁しているとは限らない。**
Tier B と第4トランシェは、この「語彙は重なるが反駁していない」型が本命。
